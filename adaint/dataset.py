...
class HDRTV1K(BaseEnhanceDataset):
    def load_annotations(self):
        ...
        Supports multiple extensions for LQ/GT, e.g.:
        LQ: a.png / b.jpg
        GT: a.png / b.tif
        """

        def find_file_with_extensions(directory, basename, exts):
            """Return the first matched file in `directory/basename.ext`."""
            for ext in exts:
                pattern = osp.join(directory, f"{basename}{ext}")
                files = glob.glob(pattern)
                if files:
                    return files[0]
            raise FileNotFoundError(f"No file found for {basename} in {directory}")

        # allowed extensions
        lq_exts = ['.png', '.jpg', '.jpeg']
        gt_exts = ['.png', '.tif', '.tiff']
        ...
        with open(self.ann_file, 'r') as fin:
            for line in fin:
                ...
                lq_path = find_file_with_extensions(self.dir_lq, basename, lq_exts)
                gt_path = find_file_with_extensions(self.dir_gt, basename, gt_exts)
